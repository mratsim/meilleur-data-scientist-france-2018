...
def xgb_validate(x_trn, x_val, y_trn, y_val, xgb_params, seed_val=0, num_rounds=4096):
    ...
    # eval
    val_score = mlogloss(y_val, y_pred)
    logger.info(f'We stopped at boosting round:  {model.best_ntree_limit}')
    logger.info(f'The mlogloss on validation set is: {val_score}\n')
...
def xgb_output(X_test, ids, classifier, n_stop, val_score, le):
  # XGBoost
